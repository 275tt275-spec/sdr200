...
#if 0
    canvas_buffer = (uint8_t*)lv_malloc(LV_CANVAS_BUF_SIZE(w, h, LV_COLOR_FORMAT_RGB565, LV_DRAW_BUF_STRIDE_ALIGN));
#else
    canvas_buffer = (uint8_t*)pvPortMalloc(LV_CANVAS_BUF_SIZE(w, h, LV_COLOR_FORMAT_RGB565, LV_DRAW_BUF_STRIDE_ALIGN));
#endif
...
#if 0
    temp_buffer = (uint8_t*)lv_malloc(LV_CANVAS_BUF_SIZE(width, height - 1, LV_COLOR_FORMAT_RGB565, LV_DRAW_BUF_STRIDE_ALIGN));
#else
    temp_buffer = (uint8_t*)pvPortMalloc(LV_CANVAS_BUF_SIZE(width, height - 1, LV_COLOR_FORMAT_RGB565, LV_DRAW_BUF_STRIDE_ALIGN));
#endif
