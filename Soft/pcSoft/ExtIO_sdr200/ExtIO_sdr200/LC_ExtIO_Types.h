...
typedef void    (__stdcall * pfnIFRateChanged)  (int IFrate);
...
typedef double  (__stdcall * pfnGetHWSR_dbl)    (void);
...
typedef int     (__stdcall * pfnExtIoSrateSelText) (int idx, char * text);     // return != 0 on error
                             // "ExtIoSrateSelText": text limited to 16 char
...
typedef void    (__stdcall * pfnSetPPMvalue) (double);
typedef double  (__stdcall * pfnGetPPMvalue) (void);
...
typedef enum
{
  ...
  , extHw_Request_ADC_Rate    = 168
  , extHw_Request_Scale_to_dBm  = 169
