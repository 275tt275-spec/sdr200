...
void remove_impulse_cache_tail(size_t bucket)
{
	...
	if (*pp) 
	{
		pffft_aligned_free((*pp)->impulse);
		pffft_aligned_free(*pp);
		*pp = NULL;
...
void free_impulse_cache(void)
{
	for (size_t b = 0; b < CACHE_BUCKETS; ++b) {
		...
		while (e) {
			...
			pffft_aligned_free(e->impulse);
			pffft_aligned_free(e);
			e = next;
		}
		_cache_heads[b] = NULL;
		_cache_counts[b] = 0;
...
float* get_impulse_cache_entry(size_t bucket, HASH_T hash, int N)
{
	if (!_run) return NULL;
...
//	LeaveCriticalSection(&_cs_use_cache);
	...
	while (e) {
		if (e->hash == hash && e->N == N) 
		{
			...
			float* imp = (float*) pffft_aligned_malloc (e->N * sizeof(complex));
...
void add_impulse_to_cache(size_t bucket, HASH_T hash, int N, float* impulse)
{
	if (!_run) return;
...
//	LeaveCriticalSection(&_cs_use_cache);
	...
	cache_entry* e = pffft_aligned_malloc (sizeof(cache_entry));
	e->hash = hash;
	e->N = N;
	e->impulse = (float *) pffft_aligned_malloc (N * sizeof(complex));
