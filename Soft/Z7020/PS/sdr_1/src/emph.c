...
EMPHP create_emphp (int run, int position, int size, int nc, int mp, float* in, float* out, int rate, int ctype, float f_low, float f_high)
{
	...
	a->f_low = f_low;
	a->f_high = f_high;
	impulse = fc_impulse (a->nc, a->f_low, a->f_high, -20.0 * log10f(a->f_high / a->f_low), 0.0, a->ctype, a->rate, 1.0 / (2.0 * a->size), 0, 0);
...
//	a->CRev = fftw_plan_dft_1d(2 * a->size, (fftw_complex *)a->product, (fftw_complex *)a->out, FFTW_BACKWARD, FFTW_PATIENT);
	...
	a->pffft_work = (float *) pffft_aligned_malloc (a->size * sizeof (complex));
}
