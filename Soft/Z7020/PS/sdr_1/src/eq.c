...
void calc_eq (EQ a)
{
	...
	a->CFor =pffft_new_setup(2 * a->size, PFFFT_COMPLEX);
...
void xeq (EQ a)
{
	...
	if (a->run)
	{
		float* work = (float *) pffft_aligned_malloc (a->size * sizeof (complex));
		memcpy (&(a->infilt[2 * a->size]), a->in, a->size * sizeof (complex));
		pffft_transform_ordered(a->CFor, a->infilt, a->product, work, PFFFT_FORWARD);
//		fftw_execute (a->CFor);
...
//		fftw_execute (a->CRev);
		pffft_transform_ordered(a->CRev, a->product, a->out, work, PFFFT_BACKWARD);
