...
void calc_pulse (GEN a)
{
	...
	a->pulse.state = 0;
	a->pulse.ctrans = (float *) pffft_aligned_malloc((a->pulse.pntrans + 1) * sizeof (float));
...
void calc_ttpulse(GEN a)
{
	...
	a->ttpulse.state = 0;
	a->ttpulse.ctrans = (float*)pffft_aligned_malloc((a->ttpulse.pntrans + 1) * sizeof(float));
...
{
	pffft_aligned_free (a->ttpulse.ctrans);
	pffft_aligned_free (a->pulse.ctrans);
}

GEN create_gen (int run, int size, float* in, float* out, int rate, int mode)
{
	GEN a = (GEN) pffft_aligned_malloc (sizeof (gen));
	a->run = run;
	a->size = size;
	a->in = in;
	a->out = out;
