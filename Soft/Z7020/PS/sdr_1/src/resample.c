...
void calc_resample (RESAMPLE a)
{
	...
	a->cpp = a->ncoef / a->L;
	a->h = (float *)pffft_aligned_malloc(a->ncoef * sizeof(float));
	impulse = fir_bandpass(a->ncoef, fc_norm_low, fc_norm_high, 1.0, 1, 0, a->gain * (float)a->L);
	i = 0;
	for (j = 0; j < a->L; j++)
		for (k = 0; k < a->ncoef; k += a->L)
			a->h[i++] = impulse[j + k];
	a->ringsize = a->cpp;
	a->ring = (float *)pffft_aligned_malloc(a->ringsize * sizeof(complex));
...
void setBuffers_resample(RESAMPLE a, float* in, float* out)
{
	a->in = in;
	a->out = out;
}
...
void setFCLow_resample (RESAMPLE a, float fc_low)
{
	if (fc_low != a->fc_low)
	{
		decalc_resample (a);
...
void setBandwidth_resample (RESAMPLE a, float fc_low, float fc_high)
{
...
{
	RESAMPLEF a = (RESAMPLEF) pffft_aligned_malloc (sizeof (resampleF));
	int x, y, z;
	int i, j, k;
	int min_rate;
	float full_rate;
	...
	a->cpp = a->ncoef / a->L;
	a->h = (float *) pffft_aligned_malloc (a->ncoef * sizeof (float));
	impulse = fir_bandpass (a->ncoef, -fc_norm, +fc_norm, 1.0f, 1, 0, (float)a->L);
	i = 0;
	for (j = 0; j < a->L; j ++)
		for (k = 0; k < a->ncoef; k += a->L)
			a->h[i++] = impulse[j + k];
	a->ringsize = a->cpp;
	a->ring = (float *) pffft_aligned_malloc (a->ringsize * sizeof (float));
	a->idx_in = a->ringsize - 1;
	a->phnum = 0;
	pffft_aligned_free (impulse);
	return a;
