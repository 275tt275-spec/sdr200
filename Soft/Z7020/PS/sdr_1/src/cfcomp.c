...
void calc_cfcwindow (CFCOMP a)
{
	...
	case 0:
		...
		for (i = 0; i < a->fsize; i++)
		{
			a->window[i] = sqrtf (0.54 - 0.46 * cosf((float)i * arg0));
			cgsum += a->window[i];
			igsum += a->window[i] * a->window[i];
		}
		coherent_gain = cgsum / (float)a->fsize;
		inherent_power_gain = igsum / (float)a->fsize;
		wmult = 1.0 / sqrtf (inherent_power_gain);
		for (i = 0; i < a->fsize; i++)
			a->window[i] *= wmult;
		a->winfudge = sqrtf (1.0 / coherent_gain);
		break;
	case 1:
		arg0 = 2.0 * PI / (float)a->fsize;
		cgsum = 0.0;
...
void calc_comp (CFCOMP a)
{
	...
	float* sary;
	a->precomplin = powf (10.0, 0.05 * a->precomp);
	a->prepeqlin  = powf (10.0, 0.05 * a->prepeq);
	fmax = 0.5 * a->rate;
	...
	for (i = 0; i < a->msize; i++)
	{
		...
		a->comp[i] = powf (10.0, 0.05 * (frac * a->gp[j + 1] + (1.0 - frac) * a->gp[j]));
		a->peq[i]  = powf (10.0, 0.05 * (frac * a->ep[j + 1] + (1.0 - frac) * a->ep[j]));
...
//	a->Rrev = fftw_plan_dft_c2r_1d(a->fsize, (fftw_complex *)a->revfftin, a->revfftout, FFTW_ESTIMATE);
	a->Rfor = pffft_plan_dft_1d(a->fsize, a->forfftin, a->forfftout, PFFFT_FORWARD, PFFFT_COMPLEX);
	a->Rrev = pffft_plan_dft_1d(a->fsize, a->revfftin, a->revfftout, PFFFT_BACKWARD, PFFFT_COMPLEX);
	a->pffft_work = pffft_aligned_malloc(a->fsize * sizeof(complex));
	calc_cfcwindow(a);
	...
	a->gain = 0.0;
	a->mmult = expf (-1.0 / (a->rate * a->ovrlp * a->mtau));
