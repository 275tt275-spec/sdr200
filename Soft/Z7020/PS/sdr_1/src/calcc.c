...
bool InterlockedBitTestAndReset(volatile long *addr,  int bit)
{
    long mask = 1L << bit;
    // Atomically OR the mask and get the old value
    long old = atomic_fetch_or(addr, mask);
    // Return true if the bit was already set
    return (old & mask) != 0;
}

bool InterlockedBitTestAndSet(volatile long *addr, int bit) {
    long mask = (1L << bit);
    // Returns the value before the OR operation
    long old = __atomic_fetch_or(addr, mask, __ATOMIC_SEQ_CST);
    return (old & mask) != 0;
}

#define InterlockedAnd(x,y) __sync_fetch_and_and(x,y)
...
void rxscheck (int rints, float* tvec, float* coef, int* info)
{
	...
	*info = 0x0000;
	...
	for (i = 0; i < rints; i++)
	{
		...
		{
			k = 4 * i + j;
			v = (float)k / (4.0 * (float)rints);
			dx = (tvec[i + 1] - tvec[i]) * (float)j / 4.0;
...
void calc (CALCC a)
{
	...
	for (i = 0; i < a->nsamps; i++)
	{
		...
		if (a->stbl && a->ctrl.running == 1 && a->scOK)
		{
			int k;
	...
	if (a->pin)	// pin
	{
		const float mval = 1.0e+00 - 1.0e-10;
		float cval, sval;
		for (i = 0; i < a->nsamps; i++)
		{
			a->cat[4 * i + 0] = a->x[i];
	...
	if (a->pin)	// tune
	{
		...
		float sf = 1.0 / (a->cm[4 * k + 0] + dx * (a->cm[4 * k + 1] + dx * (a->cm[4 * k + 2] + dx * a->cm[4 * k + 3])));
		for (i = 0; i < 4 * a->ints; i++)
			a->cm[i] *= sf;
	}
...
void pscc (int channel, int size, float* tx, float* rx)
{
	...
	{
		a->size = size;
		if (a->mox == 1 && (a->txdelay->tdelay != 0.0 || a->rxdelay->tdelay != 0.0))
		{
