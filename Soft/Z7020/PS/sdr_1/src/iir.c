...
{
	SNOTCH a = (SNOTCH) pffft_aligned_malloc (sizeof (snotch));
	a->run = run;
	a->size = size;
	a->in = in;
	a->out = out;
...
{
	SPEAK a = (SPEAK) pffft_aligned_malloc (sizeof (speak));
	a->run = run;
	a->size = size;
	a->in = in;
	a->out = out;
	...
	a->nstages = nstages;
	a->design = design;
	a->x0 = (float *) pffft_aligned_malloc (a->nstages * sizeof (complex));
	a->x1 = (float *) pffft_aligned_malloc (a->nstages * sizeof (complex));
	a->x2 = (float *) pffft_aligned_malloc (a->nstages * sizeof (complex));
	a->y0 = (float *) pffft_aligned_malloc (a->nstages * sizeof (complex));
	a->y1 = (float *) pffft_aligned_malloc (a->nstages * sizeof (complex));
	a->y2 = (float *) pffft_aligned_malloc (a->nstages * sizeof (complex));
//	InitializeCriticalSectionAndSpinCount ( &a->cs_update, 2500 );
	calc_speak (a);
	return a;
}
...
{
//	DeleteCriticalSection (&a->cs_update);
	pffft_aligned_free (a->y2);
	pffft_aligned_free (a->y1);
	pffft_aligned_free (a->y0);
	pffft_aligned_free (a->x2);
	pffft_aligned_free (a->x1);
	pffft_aligned_free (a->x0);
	pffft_aligned_free (a);
}
...
{
	int i;
	a->tmp = (float *) pffft_aligned_malloc (a->size * sizeof (complex));
	a->mix = (float *) pffft_aligned_malloc (a->size * sizeof (complex));
...
void decalc_mpeak (MPEAK a)
{
	...
	pffft_aligned_free (a->mix);
	pffft_aligned_free (a->tmp);
}
...
{
	MPEAK a = (MPEAK) pffft_aligned_malloc (sizeof (mpeak));
	a->run = run;
	a->size = size;
	a->in = in;
	a->out = out;
	...
	a->npeaks = npeaks;
	a->nstages = nstages;
	a->enable  = (int *) pffft_aligned_malloc (a->npeaks * sizeof (int));
	a->f    = (float *) pffft_aligned_malloc (a->npeaks * sizeof (float));
	a->bw   = (float *) pffft_aligned_malloc (a->npeaks * sizeof (float));
	a->gain = (float *) pffft_aligned_malloc (a->npeaks * sizeof (float));
	...
	a->pfil = (SPEAK *) pffft_aligned_malloc (a->npeaks * sizeof (SPEAK));
//	InitializeCriticalSectionAndSpinCount ( &a->cs_update, 2500 );
	calc_mpeak (a);
	return a;
}
...
void destroy_mpeak (MPEAK a)
{
	decalc_mpeak (a);
//	DeleteCriticalSection (&a->cs_update);
	pffft_aligned_free (a->pfil);
	pffft_aligned_free (a->gain);
	pffft_aligned_free (a->bw);
	pffft_aligned_free (a->f);
	pffft_aligned_free (a->enable);
	pffft_aligned_free (a);
}
...
{
	float g;
	a->x0 = (float *) pffft_aligned_malloc (a->nstages * sizeof (float));
	a->x1 = (float *) pffft_aligned_malloc (a->nstages * sizeof (float));
	a->y0 = (float *) pffft_aligned_malloc (a->nstages * sizeof (float));
	a->y1 = (float *) pffft_aligned_malloc (a->nstages * sizeof (float));
...
{
	PHROT a = (PHROT) pffft_aligned_malloc (sizeof (phrot));
	a->reverse = 0;
	a->run = run;
	a->size = size;
	a->in = in;
...
{
	BQLP a = (BQLP)pffft_aligned_malloc(sizeof(bqlp));
	a->run = run;
	a->size = size;
	a->in = in;
	a->out = out;
...
BQLP create_dbqlp(int run, int size, float* in, float* out, float rate, float fc, float Q, float gain, int nstages)
{
	...
	a->x0 = (float*)pffft_aligned_malloc(a->nstages * sizeof(float));
	a->x1 = (float*)pffft_aligned_malloc(a->nstages * sizeof(float));
	a->x2 = (float*)pffft_aligned_malloc(a->nstages * sizeof(float));
	a->y0 = (float*)pffft_aligned_malloc(a->nstages * sizeof(float));
	a->y1 = (float*)pffft_aligned_malloc(a->nstages * sizeof(float));
	a->y2 = (float*)pffft_aligned_malloc(a->nstages * sizeof(float));
//	InitializeCriticalSectionAndSpinCount(&a->cs_update, 2500);
	calc_dbqlp(a);
	return a;
}
...
{
	BQBP a = (BQBP)pffft_aligned_malloc(sizeof(bqbp));
	a->run = run;
	a->size = size;
	a->in = in;
	a->out = out;
...
{
	SPHP a = (SPHP)pffft_aligned_malloc(sizeof(sphp));
	a->run = run;
	a->size = size;
	a->in = in;
	a->out = out;
