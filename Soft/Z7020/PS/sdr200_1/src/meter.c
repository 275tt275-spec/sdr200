...
{
	a->mult_average = expf(-1.0f / (a->rate * a->tau_average));
	a->mult_peak = expf(-1.0f / (a->rate * a->tau_peak_decay));
	flush_meter(a);
}
...
void xmeter (METER a)
{
	...
	if (a->run && srun)
	{
		...
		for (i = 0; i < a->size; i++)
		{
			...
			a->avg = a->avg * a->mult_average + (1.0f - a->mult_average) * smag;
			a->peak *= a->mult_peak;
			if (smag > np) np = smag;
		}
		if (np > a->peak) a->peak = np;
		a->result[a->enum_av] = 10.0f * mlog10 (a->avg + 1.0e-40f);
		a->result[a->enum_pk] = 10.0f * mlog10 (a->peak + 1.0e-40f);
		if ((a->pgain != 0) && (a->enum_gain >= 0))
			a->result[a->enum_gain] = 20.0f * mlog10 (*a->pgain + 1.0e-40f);
	}
	else
	{
...
void setBuffers_meter (METER a, float* in)
{
	a->buff = in;
}
