...
EMPHP create_emphp (int run, int position, int size, int nc, int mp, float* in, float* out, int rate, int ctype, float f_low, float f_high)
{
	...
	a->f_low = f_low;
	a->f_high = f_high;
	impulse = fc_impulse (a->nc, a->f_low, a->f_high, -20.0f * log10f(a->f_high / a->f_low), 0.0, a->ctype, a->rate, 1.0f / (2.0f * a->size), 0, 0);
...
void setBuffers_emphp (EMPHP a, float* in, float* out)
{
	a->in = in;
	a->out = out;
...
void calc_emph (EMPH a)
{
	...
	a->mults = fc_mults(a->size, a->f_low, a->f_high, -20.0f * log10f(a->f_high / a->f_low), 0.0, a->ctype, a->rate, 1.0f / (2.0f * a->size), 0, 0);
...
void setBuffers_emph (EMPH a, float* in, float* out)
{
	decalc_emph (a);
	a->in = in;
	a->out = out;
