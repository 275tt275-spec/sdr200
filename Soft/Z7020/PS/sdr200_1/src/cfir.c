...
void calc_cfir (CFIR a)
{
	...
	impulse = cfir_impulse (a->nc, a->DD, a->R, a->Pairs, (float)a->runrate, (float)a->cicrate, a->cutoff, a->xtype, a->xbw, 1, a->scale, a->wintype);
...
CFIR create_cfir (int run, int size, int nc, int mp, float* in, float* out, int runrate, int cicrate, 
...
void setBuffers_cfir (CFIR a, float* in, float* out)
{
	decalc_cfir (a);
	a->in = in;
	a->out = out;
...
float* cfir_impulse (int N, int DD, int R, int Pairs, float runrate, float cicrate, float cutoff, int xtype, float xbw, int rtype, float scale, int wintype)
{
	...
	int i, j;
	float tmp, local_scale, ri, fn, mag = 1.0;
	float* impulse;
	...
	for (i = 0; i <= x_samps; i++)
	{
		xistion[i] = 0.5f * (cosf (phs) + 1.0f);
		phs += delta;
	}
	if ((tmp = DD * R * sinf (PI * ft / R) / sinf (PI * DD * ft)) < 0.0)	//normalize by peak gain
		tmp = -tmp;
	local_scale = scale / powf (tmp, (float)Pairs);
	if (xtype == 0)
	{
		for (i = 0, ri = offset; i < u_samps; i++, ri += 1.0)
		{
			...
			{
				if (fn == 0.0) tmp = 1.0;
				else if ((tmp = DD * R * sinf (PI * fn / R) / sinf (PI * DD * fn)) < 0.0)
					tmp = -tmp;
				mag = powf (tmp, (float)Pairs) * local_scale;
			}
			else
