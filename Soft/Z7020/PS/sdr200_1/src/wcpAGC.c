...
void loadWcpAGC (WCPAGC a)
{
	...
	a->hang_level = (a->max_input * tmp + (a->out_target / 
		(a->var_gain * a->max_gain)) * (1.0f - tmp)) * 0.637f;
...
void xwcpagc (WCPAGC a)
{
	...
	if (a->run)
	{
		...
		for (i = 0; i < a->io_buffsize; i++)
		{
			...
			if (a->pmode == 0)
				a->abs_ring[a->in_index] = max(fabsf(a->ring[2 * a->in_index + 0]), fabsf(a->ring[2 * a->in_index + 1]));
			else
				a->abs_ring[a->in_index] = sqrtf(a->ring[2 * a->in_index + 0] * a->ring[2 * a->in_index + 0] + a->ring[2 * a->in_index + 1] * a->ring[2 * a->in_index + 1]);
			...
			mult = (a->out_target - a->slope_constant * min (0.0f, log10f(a->inv_max_input * a->volts))) / a->volts;
...
{
	EnterCriticalSection (&ch[channel].csDSP);
	txa[channel].alc.p->tau_attack = (float)attack / 1000.0f;
...
{
	EnterCriticalSection (&ch[channel].csDSP);
	txa[channel].alc.p->tau_decay = (float)decay / 1000.0f;
	loadWcpAGC(txa[channel].alc.p);
	LeaveCriticalSection (&ch[channel].csDSP);
}
...
{
	EnterCriticalSection (&ch[channel].csDSP);
	txa[channel].alc.p->hangtime = (float)hang / 1000.0f;
	loadWcpAGC(txa[channel].alc.p);
	LeaveCriticalSection (&ch[channel].csDSP);
}
...
{
	EnterCriticalSection (&ch[channel].csDSP);
	txa[channel].alc.p->max_gain = powf (10.0f,(float)maxgain / 20.0f);
	loadWcpAGC(txa[channel].alc.p);
	LeaveCriticalSection (&ch[channel].csDSP);
}
...
{
	EnterCriticalSection (&ch[channel].csDSP);
	txa[channel].leveler.p->tau_attack = (float)attack / 1000.0f;
...
{
	EnterCriticalSection (&ch[channel].csDSP);
	txa[channel].leveler.p->tau_decay = (float)decay / 1000.0f;
	loadWcpAGC(txa[channel].leveler.p);
	LeaveCriticalSection (&ch[channel].csDSP);
}
...
{
	EnterCriticalSection (&ch[channel].csDSP);
	txa[channel].leveler.p->hangtime = (float)hang / 1000.0f;
	loadWcpAGC(txa[channel].leveler.p);
	LeaveCriticalSection (&ch[channel].csDSP);
}
...
{
	EnterCriticalSection (&ch[channel].csDSP);
	txa[channel].leveler.p->max_gain = powf (10.0f,(float)maxgain / 20.0f);
	loadWcpAGC(txa[channel].leveler.p);
	LeaveCriticalSection (&ch[channel].csDSP);
}
