...
void plan_firopt (FIROPT a)
{
	...
	a->pffft_work = (float*)malloc0(2 * a->size * sizeof(complex));
	for (i = 0; i < a->nfor; i++)
	{
		a->fftout[i] = (float *) malloc0 (2 * a->size * sizeof (complex));
		a->fmask[i] = (float *) malloc0 (2 * a->size * sizeof (complex));
		a->pcfor[i] = pffft_plan_dft_1d(2 * a->size, a->fftin, a->fftout[i], PFFFT_FORWARD, PFFFT_COMPLEX);
		a->maskplan[i] = pffft_plan_dft_1d(2 * a->size, a->maskgen, a->fmask[i], PFFFT_FORWARD, PFFFT_COMPLEX);
...
//		a->maskplan[i] = fftw_plan_dft_1d(2 * a->size, (fftw_complex *)a->maskgen, (fftw_complex *)a->fmask[i], FFTW_FORWARD, FFTW_PATIENT);
	...
	a->crev = pffft_plan_dft_1d(2 * a->size, a->accum, a->out, PFFFT_BACKWARD, PFFFT_COMPLEX);
...
void calc_firopt (FIROPT a)
{
	...
	for (i = 0; i < a->nfor; i++)
	{
		...
		pffftw_execute(a->maskplan[i], a->pffft_work);
	}
...
void deplan_firopt (FIROPT a)
{
	...
	for (i = 0; i < a->nfor; i++)
	{
		...
		pffftw_destroy_plan(a->pcfor[i]);
		pffftw_destroy_plan(a->maskplan[i]);
	}
...
void xfiropt (FIROPT a, int pos)
{
	if (a->run && (a->position == pos))
	{
		...
		pffftw_execute(a->pcfor[a->buffidx], a->pffft_work);
		k = a->buffidx;
...
void plan_fircore (FIRCORE a)
{
	...
	for (i = 0; i < a->nfor; i++)
	{
		...
		a->pcfor[i] = pffft_plan_dft_1d(2 * a->size, a->fftin, a->fftout[i], PFFFT_FORWARD, PFFFT_COMPLEX);
		a->maskplan[0][i] = pffft_plan_dft_1d(2 * a->size, a->maskgen, a->fmask[0][i], PFFFT_FORWARD, PFFFT_COMPLEX);
		a->maskplan[1][i] = pffft_plan_dft_1d(2 * a->size, a->maskgen, a->fmask[1][i], PFFFT_FORWARD, PFFFT_COMPLEX);
//		a->pcfor[i] = fftw_plan_dft_1d(2 * a->size, (fftw_complex *)a->fftin, (fftw_complex *)a->fftout[i], FFTW_FORWARD, FFTW_PATIENT);
...
void calc_fircore (FIRCORE a, int flip)
{
	...
	for (i = 0; i < a->nfor; i++)
	{
		...
		pffftw_execute(a->maskplan[1 - a->cset][i], a->pffft_work);
	}
	a->masks_ready = 1;
	if (flip)
	{
...
void deplan_fircore (FIRCORE a)
{
	...
	for (i = 0; i < a->nfor; i++)
	{
		...
		pffftw_destroy_plan(a->pcfor[i]);
		pffftw_destroy_plan(a->maskplan[0][i]);
		pffftw_destroy_plan(a->maskplan[1][i]);
	}
