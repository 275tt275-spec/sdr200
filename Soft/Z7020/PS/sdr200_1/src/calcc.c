...
{	// for change in ints or spi
	...
	a->env_TX = (float*)malloc0(a->nsamps * sizeof(float));
	a->env_RX = (float*)malloc0(a->nsamps * sizeof(float));
	a->x = (float*)malloc0(a->tsamps * sizeof(float));
	a->ym = (float*)malloc0(a->tsamps * sizeof(float));
	a->yc = (float*)malloc0(a->tsamps * sizeof(float));
	a->ys = (float*)malloc0(a->tsamps * sizeof(float));
	a->cat = (float*)malloc0(4 * a->nsamps * sizeof(float));

	a->t    = (float *) malloc0 ((a->ints + 1) * sizeof(float));
	a->tmap = (float *) malloc0 ((a->ints + 1) * sizeof(float));
	...
	a->cm_old = (float *) malloc0 (a->ints * 4 * sizeof (float));
	...
	a->util.pm = (float *) malloc0 (4 * a->util.ints * sizeof(float));
	a->util.pc = (float *) malloc0 (4 * a->util.ints * sizeof(float));
	a->util.ps = (float *) malloc0 (4 * a->util.ints * sizeof(float));
}
...
CALCC create_calcc (int channel, int runcal, int size, int rate, int ints, int spi, float hw_scale, 
	...
	int npsamps, float alpha)
{
	...
	a->rxdelay = create_delay (
		...
		a->rate,									// sample rate
		20.0e-09f,									// delta (delay stepsize)
		0.0);										// delay
...
{
	int i, j, k;
	float v, dx, out, x, xold;
	int intm1 = a->ints - 1;
	const float diff_thresh = 0.05f;
	a->binfo[6] = 0x0000;
	...
	for (i = 0; i < a->ints; i++)
	{
		...
		{
			k = 4 * i + j;
			v = (float)k / (4.0f * (float)a->ints);
			dx = (a->t[i + 1] - a->t[i]) * (float)j / 4.0f;
	...
	memcpy (a->cm_old, a->cm, a->ints * 4 * sizeof(float));
}

void rxscheck (int rints, float* tvec, float* coef, int* info)
{
	int i, j, k;
	int rintsm1 = rints - 1;
	float v, dx, out;
	*info = 0x0000;
	...
	for (i = 0; i < rints; i++)
	{
		...
		{
			k = 4 * i + j;
			v = (float)k / (4.0f * (float)rints);
			dx = (tvec[i + 1] - tvec[i]) * (float)j / 4.0f;
...
void calc (CALCC a)
{
	...
	{
		a->env_TX[i] = sqrtf (a->txs[2 * i + 0] * a->txs[2 * i + 0] + a->txs[2 * i + 1] * a->txs[2 * i + 1]);
		a->env_RX[i] = sqrtf (a->rxs[2 * i + 0] * a->rxs[2 * i + 0] + a->rxs[2 * i + 1] * a->rxs[2 * i + 1]);
	}
	{
		int rints, ix;
		float dx;
		...
		ix = rints - 1;
		for (i = 0; i <= rints; i++)
			tvec[i] = (float)i / (float)rints / a->hw_scale;
		dx = tvec[rints] - tvec[rints - 1];
		...
		if ((a->binfo[0] == 0) && (a->binfo[7] == 0))
			rx_scale = 1.0f / (txrxcoefs[4 * ix + 0] + dx * (txrxcoefs[4 * ix + 1] + dx * (txrxcoefs[4 * ix + 2] + dx * txrxcoefs[4 * ix + 3])));
		else
		{
			a->scOK = 0;
			goto cleanup;
		...
		if (a->stbl && _InterlockedAnd (&a->ctrl.running, 1))
			a->rx_scale = a->alpha * a->rx_scale + (1.0f - a->alpha) * rx_scale;
		else
			a->rx_scale = rx_scale;
	}
	...
	if (a->pin)	// regress
	{
		...
		for (i = 0; i < a->nsamps; i++)
		{
			max_rx = (1.0f - slope + slope * a->hw_scale * a->env_TX[i]) / a->rx_scale;
	...
	for (i = 0; i < a->nsamps; i++)
	{
		...
		if (a->stbl && _InterlockedAnd (&a->ctrl.running, 1) && a->scOK)
		{
			...
			a->ym[i] = a->alpha * ymo + (1.0f - a->alpha) * a->ym[i];
			a->yc[i] = a->alpha * yco + (1.0f - a->alpha) * a->yc[i];
			a->ys[i] = a->alpha * yso + (1.0f - a->alpha) * a->ys[i];
		}
	}

	if (a->pin)	// pin
	{
		const float mval = 1.0e+00f - 1.0e-10f;
		float cval, sval;
		for (i = 0; i < a->nsamps; i++)
		{
			a->cat[4 * i + 0] = a->x[i];
		...
		qsort(a->cat, a->nsamps, 4 * sizeof(float), fcompare);
		for (i = 0; i < a->nsamps; i++)
		{
			a->x[i]  = a->cat[4 * i + 0];
			a->ym[i] = a->cat[4 * i + 1];
	...
	{
		int k = a->ints - 1;
		float dx = a->t[a->ints] - a->t[a->ints - 1];
		float sf = 1.0f / (a->cm[4 * k + 0] + dx * (a->cm[4 * k + 1] + dx * (a->cm[4 * k + 2] + dx * a->cm[4 * k + 3])));
		for (i = 0; i < 4 * a->ints; i++)
			a->cm[i] *= sf;
	}
	...
	memcpy(a->disp.x, a->x,  a->nsamps * sizeof (float));
	memcpy(a->disp.ym, a->ym, a->nsamps * sizeof (float));
	memcpy(a->disp.yc, a->yc, a->nsamps * sizeof (float));
	memcpy(a->disp.ys, a->ys, a->nsamps * sizeof (float));
	if (a->scOK)
	{
...
//						ReleaseSemaphore(a->Sem_TurnOff, 1, 0);
			...
			case LCOLLECT:
				...
				for (i = 0; i < a->size; i++)
				{
					env = sqrtf(tx[2 * i + 0] * tx[2 * i + 0] + tx[2 * i + 1] * tx[2 * i + 1]);
...
{
EnterCriticalSection (&txa[channel].calcc.cs_update);
*peak = 1.0f / txa[channel].calcc.p->hw_scale;
LeaveCriticalSection (&txa[channel].calcc.cs_update);
}
