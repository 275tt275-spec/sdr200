...
void calc_bps (BPS a)
{
	...
	impulse = fir_bandpass(a->size + 1, a->f_low, a->f_high, a->samplerate, a->wintype, 1, 1.0f / (float)(2 * a->size));
...
BPS create_bps (int run, int position, int size, float* in, float* out, 
...
void setBuffers_bps (BPS a, float* in, float* out)
{
	decalc_bps (a);
	a->in = in;
	a->out = out;
...
void setFreqs_bps (BPS a, float f_low, float f_high)
{
	decalc_bps (a);
	a->f_low = f_low;
	a->f_high = f_high;
...
PORT
void SetRXABPSFreqs (int channel, float f_low, float f_high)
{
	float* impulse;
	BPS a1;
	EnterCriticalSection (&ch[channel].csDSP);
...
PORT
void SetTXABPSFreqs (int channel, float f_low, float f_high)
{
	float* impulse;
	BPS a;
	EnterCriticalSection (&ch[channel].csDSP);
...
BANDPASS create_bandpass (int run, int position, int size, int nc, int mp, float* in, float* out, 
	float f_low, float f_high, int samplerate, int wintype, float gain)
{
...
void setBuffers_bandpass (BANDPASS a, float* in, float* out)
{
	a->in = in;
	a->out = out;
...
void setGain_bandpass (BANDPASS a, float gain, int update)
{
	float* impulse;
	a->gain = gain;
	impulse = fir_bandpass (a->nc, a->f_low, a->f_high, a->samplerate, a->wintype, 1, a->gain / (float)(2 * a->size));
...
void CalcBandpassFilter (BANDPASS a, float f_low, float f_high, float gain)
{
	float* impulse;
...
void SetRXABandpassWindow (int channel, int wintype)
{
	...
	if ((a->wintype != wintype))
	{
		impulse = fir_bandpass (a->nc, a->f_low, a->f_high, a->samplerate, 
			wintype, 1, a->gain / (float)(2 * a->size));
		setImpulse_fircore (a->p, impulse, 0);
		_aligned_free (impulse);
		EnterCriticalSection (&ch[channel].csDSP);
		a->wintype = wintype;
