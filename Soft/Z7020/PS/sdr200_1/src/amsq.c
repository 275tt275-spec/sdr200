...
void compute_slews(AMSQ a)
{
	...
	for (i = 0; i <= a->ntup; i++)
	{
		a->cup[i] = a->muted_gain + (1.0f - a->muted_gain) * 0.5f * (1.0f - cosf (theta));
		theta += delta;
	}
	...
	{
		a->cdown[i] = a->muted_gain + (1.0f - a->muted_gain) * 0.5f * (1.0f + cosf (theta));
		theta += delta;
	}
}
...
void calc_amsq(AMSQ a)
{
	...
	a->avm = expf(-1.0f / (a->rate * a->avtau));
	a->onem_avm = 1.0f - a->avm;
	a->avsig = 0.0;
	// level change
...
AMSQ create_amsq (int run, int size, float* in, float* out, float* trigger, int rate, float avtau, 
...
void xamsq (AMSQ a)
{
	if (a->run)
	{
		...
		for (i = 0; i < a->size; i++)
		{
			sig = sqrtf (a->trigsig[2 * i + 0] * a->trigsig[2 * i + 0] + a->trigsig[2 * i + 1] * a->trigsig[2 * i + 1]);
...
void setBuffers_amsq (AMSQ a, float* in, float* out, float* trigger)
{
	a->in = in;
	a->out = out;
	a->trigger = trigger;
...
{	// dBlevel is negative
	...
	a->muted_gain = powf (10.0f, dBlevel / 20.0f);
	compute_slews(a);
	LeaveCriticalSection (&ch[channel].csDSP);
}
...
{
	float thresh = powf (10.0f, threshold / 20.0f);
	EnterCriticalSection (&ch[channel].csDSP);
	txa[channel].amsq.p->tail_thresh = 0.9f * thresh;
