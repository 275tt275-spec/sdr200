...
void xosctrl (OSCTRL a)
{
	if (a->run)
	{
		...
		for (i = 0; i < a->size; i++)
		{
			...
			a->dlenv[a->in_idx] = sqrtf (a->inbuff[2 * i + 0] * a->inbuff[2 * i + 0]		// put env in delay line 
			...
			if (a->max_env > 1.0f) divisor = 1.0f + a->osgain * (a->max_env - 1.0f);
			else                  divisor = 1.0f;
...
void setBuffers_osctrl (OSCTRL a, float* in, float* out)
{
	a->inbuff = in;
	a->outbuff = out;
}
