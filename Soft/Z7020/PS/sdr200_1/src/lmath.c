...
void dR (int n, float* r, float* y, float* z)
{
	int i, j, k;
...
BLDR create_builder(int points, int ints)
{
	...
	a->catxy = (float*)malloc0(2 * points * sizeof(float));
	a->sx    = (float*)malloc0(    points * sizeof(float));
	a->sy    = (float*)malloc0(    points * sizeof(float));
	a->h     = (float*)malloc0(    ints   * sizeof(float));
	a->p     = (int*)   malloc0(    ints   * sizeof(int));
	a->np    = (int*)   malloc0(    ints   * sizeof(int));
	a->taa   = (float*)malloc0(    ints   * sizeof(float));
	a->tab   = (float*)malloc0(    ints   * sizeof(float));
	a->tag   = (float*)malloc0(    ints   * sizeof(float));
	a->tad   = (float*)malloc0(    ints   * sizeof(float));
	a->tbb   = (float*)malloc0(    ints   * sizeof(float));
	a->tbg   = (float*)malloc0(    ints   * sizeof(float));
	a->tbd   = (float*)malloc0(    ints   * sizeof(float));
	a->tgg   = (float*)malloc0(    ints   * sizeof(float));
	a->tgd   = (float*)malloc0(    ints   * sizeof(float));
	a->tdd   = (float*)malloc0(    ints   * sizeof(float));
	int nsize = 3 * ints + 1;
	int intp1 = ints + 1;
	int intm1 = ints - 1;
	a->A     = (float*)malloc0(intp1 * intp1 * sizeof(float));
	a->B     = (float*)malloc0(intp1 * intp1 * sizeof(float));
	a->C     = (float*)malloc0(intm1 * intp1 * sizeof(float));
	a->D     = (float*)malloc0(intp1         * sizeof(float));
	a->E     = (float*)malloc0(intp1 * intp1 * sizeof(float));
	a->F     = (float*)malloc0(intm1 * intp1 * sizeof(float));
	a->G     = (float*)malloc0(intp1         * sizeof(float));
	a->MAT   = (float*)malloc0(nsize * nsize * sizeof(float));
	a->RHS   = (float*)malloc0(nsize         * sizeof(float));
	a->SLN   = (float*)malloc0(nsize         * sizeof(float));
	a->z     = (float*)malloc0(intp1         * sizeof(float));
	a->zp    = (float*)malloc0(intp1         * sizeof(float));
	a->wrk   = (float*)malloc0(nsize         * sizeof(float));
...
void xbuilder(BLDR a, int points, float* x, float* y, int ints, float* t, int* info, float* c, float ptol)
{
	...
	for (i = 0; i < ints; i++)
		for (j = a->p[i]; j < a->p[i] + a->np[i]; j++)
		{
			u = (a->sx[j] - t[i]) / a->h[i];
			v = u - 1.0f;
			alpha = (2.0f * u + 1.0f) * v * v;
			beta = u * u * (1.0f - 2.0f * v);
			gamma = a->h[i] * u * v * v;
			delta = a->h[i] * u * u * v;
			a->taa[i] += alpha * alpha;
			a->tab[i] += alpha * beta;
			...
			a->tgd[i] += gamma * delta;
			a->tdd[i] += delta * delta;
			a->D[i + 0] += 2.0f * a->sy[j] * alpha;
			a->D[i + 1] += 2.0f * a->sy[j] * beta;
			a->G[i + 0] += 2.0f * a->sy[j] * gamma;
			a->G[i + 1] += 2.0f * a->sy[j] * delta;
		}
	for (i = 0; i < ints; i++)
	{
		a->A[(i + 0) * intp1 + (i + 0)] += 2.0f * a->taa[i];
		a->A[(i + 1) * intp1 + (i + 1)] = 2.0f * a->tbb[i];
		a->A[(i + 0) * intp1 + (i + 1)] = 2.0f * a->tab[i];
		a->A[(i + 1) * intp1 + (i + 0)] = 2.0f * a->tab[i];
		a->B[(i + 0) * intp1 + (i + 0)] += 2.0f * a->tag[i];
		a->B[(i + 1) * intp1 + (i + 1)] = 2.0f * a->tbd[i];
		a->B[(i + 0) * intp1 + (i + 1)] = 2.0f * a->tbg[i];
		a->B[(i + 1) * intp1 + (i + 0)] = 2.0f * a->tad[i];
		a->E[(i + 0) * intp1 + (i + 0)] += 2.0f * a->tgg[i];
		a->E[(i + 1) * intp1 + (i + 1)] = 2.0f * a->tdd[i];
		a->E[(i + 0) * intp1 + (i + 1)] = 2.0f * a->tgd[i];
		a->E[(i + 1) * intp1 + (i + 0)] = 2.0f * a->tgd[i];
	}
	for (i = 0; i < intm1; i++)
	{
		a->C[i * intp1 + (i + 0)] = +3.0f * a->h[i + 1] / a->h[i];
		a->C[i * intp1 + (i + 2)] = -3.0f * a->h[i] / a->h[i + 1];
		a->C[i * intp1 + (i + 1)] = -a->C[i * intp1 + (i + 0)] - a->C[i * intp1 + (i + 2)];
		a->F[i * intp1 + (i + 0)] = a->h[i + 1];
		a->F[i * intp1 + (i + 1)] = 2.0f * (a->h[i] + a->h[i + 1]);
	...
	for (i = 0; i < ints; i++)
	{
		c[4 * i + 0] = a->z[i];
		c[4 * i + 1] = a->zp[i];
		c[4 * i + 2] = -3.0f / (a->h[i] * a->h[i]) * (a->z[i] - a->z[i + 1]) - 1.0f / a->h[i] * (2.0f * a->zp[i] + a->zp[i + 1]);
		c[4 * i + 3] = 2.0f / (a->h[i] * a->h[i] * a->h[i]) * (a->z[i] - a->z[i + 1]) + 1.0f / (a->h[i] * a->h[i]) * (a->zp[i] + a->zp[i + 1]);
	}
cleanup:
	return;
}
