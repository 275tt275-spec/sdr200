	...
	if (_InterlockedAnd(&a->run, 1))
	{
		...
		for (i = 0; i < a->size; i++)
		{
			...
			case RUN:
				...
				break;
			case BEGIN:
				PRE0 = (1.0f - a->cup[a->count]) * I + a->cup[a->count] * PRE0;
				PRE1 = (1.0f - a->cup[a->count]) * Q + a->cup[a->count] * PRE1;
				if (a->count++ == a->ntup)
				{
					a->state = RUN;
					a->count = 0;
			...
			case SWAP:
				...
				PRE0 = (1.0f - a->cup[a->count]) * ym * (I * yc - Q * ys) + a->cup[a->count] * PRE0;
				PRE1 = (1.0f - a->cup[a->count]) * ym * (I * ys + Q * yc) + a->cup[a->count] * PRE1;
				if (a->count++ == a->ntup)
				{
					a->state = RUN;
					a->count = 0;
				...
				break;
			case END:
				PRE0 = (1.0f - a->cup[a->count]) * PRE0 + a->cup[a->count] * I;
				PRE1 = (1.0f - a->cup[a->count]) * PRE1 + a->cup[a->count] * Q;
				if (a->count++ == a->ntup)
				{
					a->state = DONE;
					a->count = 0;
...
void setBuffers_iqc (IQC a, float* in, float* out)
{
	a->in = in;
	a->out = out;
}
