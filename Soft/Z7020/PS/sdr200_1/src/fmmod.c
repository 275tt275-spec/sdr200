...
{
	// ctcss gen
	a->tscale = 1.0f / (1.0f + a->ctcss_level);
	a->tphase = 0.0;
...
FMMOD create_fmmod (int run, int size, float* in, float* out, int rate, float dev, float f_low, float f_high, 
	int ctcss_run, float ctcss_level, float ctcss_freq, int bp_run, int nc, int mp)
{
	...
	a->mp = mp;
	calc_fmmod (a);
	impulse = fir_bandpass(a->nc, -a->bp_fc, +a->bp_fc, a->samplerate, 0, 1, 1.0f / (2 * a->size));
	...
	if (a->run)
	{
		...
		for (i = 0; i < a->size; i++)
		{
			if (a->ctcss_run)
			{
				...
				a->out[2 * i + 0] = a->tscale * (a->in[2 * i + 0] + a->ctcss_level * cosf (a->tphase));
			}
			dp = a->out[2 * i + 0] * a->sdelta;
			a->sphase += dp;
			if (a->sphase >= TWOPI) a->sphase -= TWOPI;
			if (a->sphase <   0.0 ) a->sphase += TWOPI;
			a->out[2 * i + 0] = 0.7071f * cosf (a->sphase);
			a->out[2 * i + 1] = 0.7071f * sinf (a->sphase);
...
void setBuffers_fmmod (FMMOD a, float* in, float* out)
{
	a->in = in;
	a->out = out;
	calc_fmmod (a);
...
void SetTXAFMDeviation (int channel, float deviation)
{
	...
	float* impulse = fir_bandpass (a->nc, -bp_fc, +bp_fc, a->samplerate, 0, 1, 1.0 / (2 * a->size));
