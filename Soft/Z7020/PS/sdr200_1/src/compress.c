...
void xcompressor (COMPRESSOR a)
{
	...
	if (a->run)
		...
		{
			mag = sqrtf(a->inbuff[2 * i + 0] * a->inbuff[2 * i + 0] + a->inbuff[2 * i + 1] * a->inbuff[2 * i + 1]);
			if (a->gain * mag > 1.0)
...
void setBuffers_compressor (COMPRESSOR a, float* in, float* out)
{
	a->inbuff = in;
	a->outbuff = out;
}
...
{
	EnterCriticalSection (&ch[channel].csDSP);
	txa[channel].compressor.p->gain = powf (10.0f, gain / 20.0f);
	LeaveCriticalSection (&ch[channel].csDSP);
}
