...
float* fc_impulse (int nc, float f0, float f1, float g0, float g1, int curve, float samplerate, float scale, int ctfmode, int wintype)
{
	...
	float* imp = get_impulse_cache_entry(FC_CACHE, h, nc);
	if (imp) return imp;
	//

	float* A  = (float *) malloc0 ((nc / 2 + 1) * sizeof (float));
	int i;
	float fn, f;
	float* impulse;
	int mid = nc / 2;
	float g0_lin = powf(10.0f, g0 / 20.0f);
	if (nc & 1)
	{
		for (i = 0; i <= mid; i++)
		{
			fn = (float)i / (float)mid;
			f = fn * samplerate / 2.0f;
			switch (curve)
			{
	...
	if (ctfmode == 0)
	{
		...
		if (nc & 1)
		{
			low  = (int)(2.0f * f0 / samplerate * mid);
			high = (int)(2.0f * f1 / samplerate * mid + 0.5f);
			lowmag = A[low];
			highmag = A[high];
	...
	if (nc & 1)
		impulse = fir_fsamp_odd(nc, A, 1, 1.0f, wintype);
	else
		impulse = fir_fsamp(nc, A, 1, 1.0f, wintype);
...
float* fc_mults (int size, float f0, float f1, float g0, float g1, int curve, float samplerate, float scale, int ctfmode, int wintype)
{
